...
void demo_init(void)
{
    ...
    printk("\n\n");
    printk("CEVA FSP200 Calibration Demo.\n");
...
{
    // Prompt for start
    printk("Put module in start orientation, press ENTER.\n");
    printk("> ");
    ...
    switch (calState)
    {
        case CAL_WAIT_START:
            ...
            if (pressedEnter())
            {
                ...
                if (status != SH2_OK)
                {
                    ...
                    printk("Error from sh2_startCal: %d\n", status);
                    calState = CAL_DONE;
                }
                else
                {
                    // Proceed to next step: final orientation
                    printk("Rotate module to final orientation, press ENTER.\n");
                    printk("> ");
        ...
        case CAL_WAIT_FINISH:
            ...
            if (pressedEnter())
            {
                ...
                {
                    // End calibration process with error
                    printk("Error from sh2_finishCal: %d\n", status);
                }
                else if (calStatus != 0)
                {
                    // Calibration process ended
                    printk("Calibration completed with status: %d\n", calStatus);
                    printk("%s\n", calStatusMsg[calStatus]);
                }
                else
                {
                    printk("Calibration completed successfully.\n");
                }
                calState = CAL_DONE;
            }
            break;
