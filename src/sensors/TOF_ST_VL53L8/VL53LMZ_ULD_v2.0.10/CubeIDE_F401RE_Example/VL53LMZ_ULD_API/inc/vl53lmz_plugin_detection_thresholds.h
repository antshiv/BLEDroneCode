...
#ifndef VL53LMZ_PLUGIN_DETECTION_THRESHOLDS_H_
#define VL53LMZ_PLUGIN_DETECTION_THRESHOLDS_H_
...
#define VL53LMZ_NB_THRESHOLDS				((uint8_t)64U)

/**
 * @brief Inner Macro for API. Not for user, only for development.
 */

#define VL53LMZ_DCI_DET_THRESH_CONFIG			((uint16_t)0x5488U)
#define VL53LMZ_DCI_DET_THRESH_GLOBAL_CONFIG	((uint16_t)0xB6E0U)
#define VL53LMZ_DCI_DET_THRESH_START			((uint16_t)0xB6E8U)
#define VL53LMZ_DCI_DET_THRESH_VALID_STATUS	    ((uint16_t)0xB9F0U)

/**
 * @brief Macro VL53LMZ_LAST_THRESHOLD is used to indicate the end of checkers
 * programming.
 */

#define VL53LMZ_LAST_THRESHOLD				((uint8_t)128U)
...
#define VL53LMZ_DISTANCE_MM				    ((uint8_t)1U)
#define VL53LMZ_SIGNAL_PER_SPAD_KCPS		((uint8_t)2U)
#define VL53LMZ_RANGE_SIGMA_MM				((uint8_t)4U)
#define VL53LMZ_AMBIENT_PER_SPAD_KCPS		((uint8_t)8U)
#define VL53LMZ_NB_TARGET_DETECTED			((uint8_t)9U)
#define VL53LMZ_TARGET_STATUS				((uint8_t)12U)
#define VL53LMZ_NB_SPADS_ENABLED			((uint8_t)13U)
#define VL53LMZ_MOTION_GLOBAL_INDICATOR		((uint8_t)16U)
#define VL53LMZ_MOTION_INDICATOR			((uint8_t)19U)
...
#define VL53LMZ_IN_WINDOW					((uint8_t)0U)
#define VL53LMZ_OUT_OF_WINDOW				((uint8_t)1U)
#define VL53LMZ_LESS_THAN_EQUAL_MIN_CHECKER ((uint8_t)2U)
#define VL53LMZ_GREATER_THAN_MAX_CHECKER	((uint8_t)3U)
#define VL53LMZ_EQUAL_MIN_CHECKER			((uint8_t)4U)
#define VL53LMZ_NOT_EQUAL_MIN_CHECKER		((uint8_t)5U)
...
#define VL53LMZ_OPERATION_NONE				((uint8_t)0U)
#define VL53LMZ_OPERATION_OR				((uint8_t)0U)
#define VL53LMZ_OPERATION_AND				((uint8_t)2U)

/**
 * @brief Structure VL53LMZ_DetectionThresholds contains a single threshold.
 * This structure  is never used alone, it must be used as an array of 64
 * thresholds (defined by macro VL53LMZ_NB_THRESHOLDS).
 */
...
typedef struct {
	...
	/* Measurement to catch (VL53LMZ_MEDIAN_RANGE_MM,...)*/
	uint8_t		measurement;
	/* Windows type (VL53LMZ_IN_WINDOW, VL53LMZ_OUT_WINDOW, ...) */
	uint8_t		type;
	/* Zone id. Please read VL53L5 user manual to find the zone id.Set macro
	 * VL53LMZ_LAST_THRESHOLD to indicates the end of checkers */
	uint8_t		zone_num;
	/* Mathematics operation (AND/OR). The first threshold is always OR.*/
	uint8_t		mathematic_operation;
}VL53LMZ_DetectionThresholds;
...
uint8_t vl53lmz_get_detection_thresholds_enable(
...
uint8_t vl53lmz_set_detection_thresholds_enable(
		VL53LMZ_Configuration		*p_dev,
		uint8_t				enabled);
 ...
 * @param (VL53LMZ_Configuration) *p_dev : VL53L5CX configuration structure.
 * @param (VL53LMZ_DetectionThresholds) *p_thresholds : Array of 64 thresholds.
 * @return (uint8_t) status : 0 if programming is OK
 */

uint8_t vl53lmz_get_detection_thresholds(
		VL53LMZ_Configuration		*p_dev,
		VL53LMZ_DetectionThresholds	*p_thresholds);
...
uint8_t vl53lmz_set_detection_thresholds(
		VL53LMZ_Configuration		*p_dev,
		VL53LMZ_DetectionThresholds	*p_thresholds);
 ...
 * note that vl53lmz_stop_ranging() function needs to be used after interrupt raised
 * for a clean stop.
...
uint8_t vl53lmz_get_detection_thresholds_auto_stop(
		VL53LMZ_Configuration		*p_dev,
		uint8_t				*p_auto_stop);
