...
/***********************************/
/*    VL53LMZ ULD power mode      */
/***********************************/
/*
* This example shows the possibility of VL53LMZ to change power mode. It
* initializes the VL53LMZ ULD, set a configuration, change the power mode, and
...
int example4(void)
{
	...
	*/
	 ...
	 */
	status = vl53lmz_set_power_mode(&Dev, VL53LMZ_POWER_MODE_SLEEP);
	if(status)
	{
		printf("vl53lmz_set_power_mode failed, status %u\n", status);
		return status;
	}
	printf("VL53LMZ is now sleeping\n");
	...
	status = vl53lmz_set_power_mode(&Dev, VL53LMZ_POWER_MODE_WAKEUP);
	if(status)
	{
		printf("vl53lmz_set_power_mode failed, status %u\n", status);
		return status;
	}
	printf("VL53LMZ is now waking up\n");
