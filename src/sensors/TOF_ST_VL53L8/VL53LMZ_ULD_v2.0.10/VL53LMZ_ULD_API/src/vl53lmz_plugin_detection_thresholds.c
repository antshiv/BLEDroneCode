...
#include "../inc/vl53lmz_plugin_detection_thresholds.h"
		...
		uint8_t				*p_enabled)
{
	...
	status |= vl53lmz_dci_read_data(p_dev, (uint8_t*)p_dev->temp_buffer,
			VL53LMZ_DCI_DET_THRESH_GLOBAL_CONFIG, 8);
		...
		uint8_t				enabled)
{
	...
	status |= vl53lmz_dci_replace_data(p_dev, p_dev->temp_buffer,
			VL53LMZ_DCI_DET_THRESH_CONFIG, 20,
			(uint8_t*)&tmp, 1, 0x11);
		...
		VL53LMZ_DetectionThresholds	*p_thresholds)
{
	...
	status |= vl53lmz_dci_read_data(p_dev, (uint8_t*)p_thresholds,
			VL53LMZ_DCI_DET_THRESH_START,
                        (uint16_t)VL53LMZ_NB_THRESHOLDS
			*(uint16_t)sizeof(VL53LMZ_DetectionThresholds));

	for(i = 0; i < (uint8_t)VL53LMZ_NB_THRESHOLDS; i++)
	{
		switch(p_thresholds[i].measurement)
		{
			case VL53LMZ_DISTANCE_MM:
				p_thresholds[i].param_low_thresh  /= 4;
				p_thresholds[i].param_high_thresh /= 4;
				break;
			case VL53LMZ_SIGNAL_PER_SPAD_KCPS:
				p_thresholds[i].param_low_thresh  /= 2048;
				p_thresholds[i].param_high_thresh /= 2048;
				break;
			case VL53LMZ_RANGE_SIGMA_MM:
				p_thresholds[i].param_low_thresh  /= 128;
				p_thresholds[i].param_high_thresh /= 128;
				break;
			case VL53LMZ_AMBIENT_PER_SPAD_KCPS:
				p_thresholds[i].param_low_thresh  /= 2048;
				p_thresholds[i].param_high_thresh /= 2048;
				break;
			case VL53LMZ_NB_SPADS_ENABLED:
				p_thresholds[i].param_low_thresh  /= 256;
				p_thresholds[i].param_high_thresh /= 256;
				break;
			case VL53LMZ_MOTION_INDICATOR:
		...
		VL53LMZ_DetectionThresholds	*p_thresholds)
{
	...
	status |= vl53lmz_dci_write_data(p_dev, (uint8_t*)grp_valid_target_cfg,
			VL53LMZ_DCI_DET_THRESH_VALID_STATUS,
			(uint16_t)sizeof(grp_valid_target_cfg));

	/* Set thresholds configuration */
	status |= vl53lmz_dci_write_data(p_dev, (uint8_t*)p_thresholds,
			VL53LMZ_DCI_DET_THRESH_START,
			(uint16_t)(VL53LMZ_NB_THRESHOLDS
			*sizeof(VL53LMZ_DetectionThresholds)));
